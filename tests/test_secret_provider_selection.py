...
class TestProviderSelection:
    ...
    def test_selected_provider_defaults_to_builtin(self, monkeypatch):
        monkeypatch.delenv("LUCENT_SECRET_PROVIDER", raising=False)
        assert get_selected_provider_name() == "builtin"

    def test_initialize_vault_selected(self, monkeypatch):
        monkeypatch.setenv("LUCENT_SECRET_PROVIDER", "vault")
        monkeypatch.setenv("VAULT_ADDR", "http://vault:8200")
        monkeypatch.setenv("VAULT_TOKEN", "test-token")
        provider = initialize_secret_provider(object())
        assert SecretRegistry.get() is provider

    def test_initialize_aws_selected(self, monkeypatch):
    ...
    def test_initialize_azure_selected(self, monkeypatch):
